...
class JoomSportClassMatchdayRound{
    ...
    public function getViewEdit() {
        ...
        // Vérifier les droits de l'utilisateur pour les équipes à modérer
        ...
        // Requête pour récupérer les matchs correspondants
        ...
        <div class="jsOverXdiv">
            ...
            <table class="mglTable" id="mglMatchDay">
                ...
                <tr>
                    ...
                    <?php
                    // Boucle pour afficher les champs supplémentaires (extra fields)
                ...
                for ($intA = 0; $intA < count($matches->posts); $intA++) {
                    ...
                    <tr>
                        ...
                        <td nowrap="nowrap">
                            <input type="number" name="home_score[]" class="mglScore jsNumberNotNegative" value="<?php echo esc_attr($home_score); ?>">
                            : <input type="number" name="away_score[]" class="mglScore jsNumberNotNegative" value="<?php echo esc_attr($away_score); ?>">
                        </td>
                        <td>
                            <?php echo esc_html(get_the_title($away_team)); ?><input type="hidden" name="away_team[]" value="<?php echo esc_attr($away_team); ?>"></td>
                        <td>
                            <!-- Champ pour saisir la cote de la victoire -->
                            <input type="text" name="win_odds[]" value="<?php echo esc_attr($win_odds); ?>" class="jsNumberNotNegative" placeholder="Win Odds">
                            <!-- Champ pour saisir la cote de la défaite -->
                            <input type="text" name="loose_odds[]" value="<?php echo esc_attr($loose_odds); ?>" class="jsNumberNotNegative" placeholder="Loose Odds">
                            <!-- Champ pour saisir la cote du match nul -->
                            <input type="text" name="draw_odds[]" value="<?php echo esc_attr($draw_odds); ?>" class="jsNumberNotNegative" placeholder="Draw Odds">
                        </td>
                        <?php
                        if ($enabla_extra && JoomsportSettings::get('mdf_et')) {
                        ...
                        if (JoomsportSettings::get('mdf_time', 1)) {
                            echo '<td><input type="time" name="m_time[]" maxlength="5" size="12" value="'.esc_attr($m_time).'" /></td>';
                        }
                ...
                if (current_user_can('edit_jscp_matchs') && $canAddMatches) {
                    ...
                    <tr>
                        ...
                        <td nowrap="nowrap">
                            <input type="number" name="set_score_home" id="set_score_home" class="mglScore jsNumberNotNegative" />
                            :<input type="number" name="set_score_away" id="set_score_away" class="mglScore jsNumberNotNegative" />
                        </td>
                        <td>
                        ...
                        <td>
                            <!-- Champ pour saisir la cote de la victoire -->
                            <input type="text" name="set_win_odds" class="jsNumberNotNegative" placeholder="Win Odds">
                            <!-- Champ pour saisir la cote de la défaite -->
                            <input type="text" name="set_loose_odds" class="jsNumberNotNegative" placeholder="loose Odds">
                            <!-- Champ pour saisir la cote du match nul -->
                            <input type="text" name="set_draw_odds" class="jsNumberNotNegative" placeholder="Draw Odds">
                        </td>
                        <?php
                        if ($enabla_extra && JoomsportSettings::get('mdf_et')) {
                            echo '<td class="col-extra-time" nowrap="nowrap">';
                        ...
                        for ($intA = 0; $intA < count($efields); $intA++) {
                            $ef = $efields[$intA];
                            if (JoomsportSettings::get('extra_'.$ef->id)) {
                                JoomSportHelperEF::getEFInput($ef, '', 'ef_'.$ef->id.'_foot', true, false);
                                if ($ef->field_type == '2') {
                                } else {
                                    echo '<td></td>';
                                }
                        ...
                        ?>
                        <td><input type="submit" name="joomsport_add_match" class="button" value="<?php echo esc_attr(__('Add match', 'joomsport-sports-league-results-management')); ?>"></td>
                    </tr>
                    </tfoot>
                    <?php
                }
        ...
        if(isset($_POST['match_id']) && count($_POST['match_id'])){
            for($intA = 0; $intA < count($_POST['match_id']); $intA++){
                ...
                if($post_id){
                    ...
                    if (isset($_POST['win_odds']) && isset($_POST['loose_odds']) && isset($_POST['draw_odds'])) {
                        $win_odds = array_map('sanitize_text_field', $_POST['win_odds']);
                        $loose_odds = array_map('sanitize_text_field', $_POST['loose_odds']);
                        $draw_odds = array_map('sanitize_text_field', $_POST['draw_odds']);
                    
                        foreach ($win_odds as $index => $odd) {
                                $match_id = sanitize_text_field($_POST['match_id'][$index]);
                                update_post_meta($match_id, 'win_odds', $odd);
                        }
                    
                         foreach ($loose_odds as $index => $odd) {
                                $match_id = sanitize_text_field($_POST['match_id'][$index]);
                                update_post_meta($match_id, 'loose_odds', $odd);
                        }
                    
                        foreach ($draw_odds as $index => $odd) {
                                $match_id = sanitize_text_field($_POST['match_id'][$index]);
                                update_post_meta($match_id, 'draw_odds', $odd);
                        }
                    
                    }else {
                        // Déclencher une erreur "Cote non valide"
                        trigger_error('Cote non valide', E_USER_ERROR);
                    }
        ...
        if(!JoomSportUserRights::canAddMatch($this->_seasonID, $output["set_home_team"], $output["set_away_team"])){
            ...
            die();
                ...
                if($post_id){
                    ...
                    //sauvegarde de cotes debut 
                    if (isset($output["set_win_odds"])) {
                        update_post_meta($post_id, 'win_odds', sanitize_text_field($output["set_win_odds"]));
                    }
                    if (isset($output["set_loose_odds"])) {
                        update_post_meta($post_id, 'loose_odds', sanitize_text_field($output["set_loose_odds"]));
                    }
                    if (isset($output["set_draw_odds"])) {
                        update_post_meta($post_id, 'draw_odds', sanitize_text_field($output["set_draw_odds"]));
                    }
                    // sauvegardes cotes fin 
                    update_post_meta($post_id, '_joomsport_groupID', $groupID);
                    update_post_meta($post_id, '_joomsport_seasonid', $this->_seasonID);
